...
class PostDto {
  ...
  factory PostDto.fromHtml(Element postElement) {
    // Parse ID
    ...
    final submittedOn =
        dateString != null ? _parseDateString(dateString) : null;
    ...
    final filename = postElement
        .querySelector('a.thumb_image')
        ?.attributes['href']
        ?.split('/')
        .last;
...
(int?, int?, double?) _parseDimensions(String dimensions) {
  try {
    final match =
        RegExp(r'(\d+)x(\d+).*?([\d.]+)\s+MPixel').firstMatch(dimensions);
